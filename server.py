...
class GamepadBridge:
    ...
    def _create_gamepad(self, profile: str):
        ...
        if profile in ("ps", "ps5"):
            logging.info("Using VDS4Gamepad (PlayStation) profile for slot %s", self.slot)
            return vg.VDS4Gamepad()

        logging.info("Using VX360Gamepad (Xbox) profile for slot %s", self.slot)
    ...
    def _resolve_button_enum(self, logical_button: str):
        ...
        if enum_obj is None:
            if self.debug_output:
                logging.error("[DEBUG] Slot %s: Could not find enum class %s for profile %s", 
                             self.slot, "DS4_BUTTONS" if self.profile == "ps5" else "XUSB_BUTTON", self.profile)
            return None
        ...
        if self.debug_output:
            logging.error("[DEBUG] Slot %s: Could not find button enum '%s' in %s for profile %s", 
                         self.slot, candidates.get(logical_button, ["?"])[0], 
                         "DS4_BUTTONS" if self.profile == "ps5" else "XUSB_BUTTON", self.profile)
        return None
    ...
    def _set_logical_button(self, logical_button: str, should_press: bool) -> None:
        ...
        if logical_button in ("lt", "rt"):
            ...
            if self.debug_output:
                logging.info("[DEBUG] Slot %s: %s trigger %s", self.slot, logical_button.upper(), "PRESSED" if should_press else "RELEASED")
            return

        button = self._resolve_button_enum(logical_button)
        if button is None:
            if self.debug_output:
                logging.warning("[DEBUG] Slot %s: Could not resolve button enum for '%s'", self.slot, logical_button)
            return
        ...
        self.logical_pressed[logical_button] = should_press
        if self.debug_output:
            logging.info("[DEBUG] Slot %s: %s button [%s] %s (profile: %s)", self.slot, logical_button.upper(), button, "PRESSED" if should_press else "RELEASED", self.profile)

    def apply_buttons(self, button_mask: int) -> None:
        mask = int(button_mask)
        if self.debug_output:
            pressed_buttons = []
            if (mask & BUTTON_SOUTH) != 0: pressed_buttons.append("SOUTH(A)")
            if (mask & BUTTON_EAST) != 0: pressed_buttons.append("EAST(B)")
            if (mask & BUTTON_WEST) != 0: pressed_buttons.append("WEST(X)")
            if (mask & BUTTON_NORTH) != 0: pressed_buttons.append("NORTH(Y)")
            if (mask & BUTTON_LB) != 0: pressed_buttons.append("LB(L1)")
            if (mask & BUTTON_RB) != 0: pressed_buttons.append("RB(R1)")
            if (mask & BUTTON_LT) != 0: pressed_buttons.append("LT(L2)")
            if (mask & BUTTON_RT) != 0: pressed_buttons.append("RT(R2)")
            if (mask & BUTTON_BACK) != 0: pressed_buttons.append("BACK(Share)")
            if (mask & BUTTON_START) != 0: pressed_buttons.append("START(Menu)")
            if (mask & BUTTON_LS) != 0: pressed_buttons.append("LS(L3)")
            if (mask & BUTTON_RS) != 0: pressed_buttons.append("RS(R3)")
            if (mask & BUTTON_DPAD_UP) != 0: pressed_buttons.append("DPAD_UP")
            if (mask & BUTTON_DPAD_DOWN) != 0: pressed_buttons.append("DPAD_DOWN")
            if (mask & BUTTON_DPAD_LEFT) != 0: pressed_buttons.append("DPAD_LEFT")
            if (mask & BUTTON_DPAD_RIGHT) != 0: pressed_buttons.append("DPAD_RIGHT")
            if pressed_buttons:
                logging.info("[DEBUG] Slot %s: Button mask 0x%X -> %s", self.slot, mask, ", ".join(pressed_buttons))
    ...
    try:
        async for message in websocket:
            ...
            if text.startswith("D:"):
                debug_mode = text[2:].strip().lower()
                session.bridge.gamepad.debug_output = debug_mode in ("1", "true", "on")
                logging.info("Debug output %s for slot %s", "ENABLED" if session.bridge.gamepad.debug_output else "DISABLED", session.slot)
                continue
...
async def main() -> None:
    ...
    logging.info("=" * 80)
    logging.info("Virtual Controller Server Starting")
    logging.info("=" * 80)
    
    if vg is None:
        logging.warning("⚠️  vgamepad library NOT found - Running in NO-OP mode!")
        logging.warning("   Button inputs will NOT reach your computer.")
        logging.warning("   Install it with: pip install vgamepad")
    else:
        logging.info("✓ vgamepad library loaded successfully")
